...
# RETRIEVAL FUNCTION
# -----------------------------
def retrieve_docs(query=None, location=None, president=None, k=5):
    results = []
    ...
    if query:
        ...
        # Step 1: get top doc per president
        per_president = (
            df.groupby("president", as_index=False)
              .first()  # since already sorted, this is top per president
        )

        selected = per_president.to_dict("records")

        # Step 2: fill remaining slots with next best docs
        if len(selected) < k:
        ...
        # No scoring → just ensure diversity
        df = df.copy()
        df["score"] = 1.0

        # Step 1: one per president
...
"""
    ...
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "You are a geopolitical analyst specializing in U.S. presidential rhetoric."},
            {"role": "user", "content": prompt}
        ],
        temperature=0.4,
        max_tokens=500
...
with tab1:
    ...
    with right_col:
        selected_country = st.selectbox(
            "Select a country",
            [""] + sorted(country_counts["country"].unique()),
        ...
        if st.session_state.clicked_country:
            ...
            docs = retrieve_docs(
                ...
                president=(None if selected_president=="All Presidents" else selected_president)
            )
    ...
    if docs:
                context = "\n\n".join(
                    f"{d['president']} ({d['date']}): {d['text'][:500]}"
                    for d in docs
                )
